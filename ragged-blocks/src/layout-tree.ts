 ...
 */
export function measureLayoutTree(
	tree: LayoutTree,
	measure: (text: string) => Rect
): LayoutTree<WithMeasurements> {
	switch (tree.type) {
		case 'Newline':
			return tree;
		case 'Atom': {
 ...
 */
export function* eachAtomWithInheritedStyles<A extends Ann>(
	tree: LayoutTree<A>
): IterableIterator<Atom<WithStyles<A>>> {
...
export class FragmentBoundingBoxesRendering extends Render {
	...
	render(svg: Svg, _sty: SVGStyle) {
		for (const frag of this.layoutResult.fragmentsInfo()) {
			svg
				.rect(width(frag.rect), height(frag.rect))
