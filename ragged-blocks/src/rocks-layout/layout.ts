 ...
 */
function wrapRegionWithAdvance(
	backing: Backing,
	region: RegionWithAdvance,
	padding: number,
	translate: boolean
) {
...
export class RocksLayout implements alt.Layout {
	...
	async layout(
		layoutTree: alt.LayoutTree<alt.WithMeasurements>
	): Promise<UnsimplifiedRocksLayoutResult> {
		...
		for (const line of layout) {
			...
			const adjustedOffset =
				lastLineOffset + Math.max(effectiveLeading, this.settings.idealLeading);
	...
	async layout(
		...
		for (const line of layout) {
			...
			for (const fragment of line.slice(1)) {
				...
				const advance = RocksLayoutWithPins.advance(
					backing,
					timetable,
					lastFragmentIndex,
					fragment.idx
...
class OutlinedRocksLayoutResult extends Render implements FragmentsInfo {
	...
	constructor(
		layoutTree: LayoutTree<WithRegions<WithOutlines>>,
		unsimplifiedResult: UnsimplifiedRocksLayoutResult
	) {
		super();
		this.layoutTree = layoutTree;
...
export class OutlinedRocksLayoutSettings implements ViewSettings {
	...
	clone() {
		return new OutlinedRocksLayoutSettings(
			this.translateWraps,
			this.idealLeading,
			this.enableSimplification
		);
	}
}
...
export class OutlinedRocksLayout implements alt.Layout {
	...
	async layout(
		layoutTree: alt.LayoutTree<alt.WithMeasurements>
	): Promise<OutlinedRocksLayoutResult> {
    ...
    */
		...
		const go = (
			root: LayoutTree<WithRegions>,
			outline: Polygon
		): LayoutTree<WithRegions<WithOutlines>> => {
			switch (root.type) {
				case 'JoinH':
				case 'JoinV': {
