...
export function regionOfLayoutTree(layoutTree: LayoutTree<WithRegions>): Region {
	switch (layoutTree.type) {
		...
		case 'JoinV':
		case 'JoinH':
			return layoutTree.region;
		case 'Spacer':
		case 'Atom':
			return regionFromStackRef(layoutTree.stackRef);
	}
}
...
export class Timetable {
	 ...
	 */
	static fromLayoutTree<A extends Ann>(
		layoutTree: LayoutTree<A>
	): [Timetable, LayoutTree<WithRegions<A>>] {
		...
		return [new Timetable(columns, maxDepth), withRegions];
	}
