 ...
 * to fragment ranges.
 */
function buildFragmentVector(
	layoutTree: LayoutTree<WithMeasurements>
): LayoutGuts<WithMeasurements<WithFragmentRanges>> {
	/**
	 * Produce a unique ID.
	 */
	const nextUid = (() => {
	...
	const go = (
		root: LayoutTree<WithMeasurements>
	): LayoutTree<WithMeasurements<WithFragmentRanges>> => {
		switch (root.type) {
			case 'Newline': {
			...
			case 'Node': {
				...
				for (let i = beginIndex; i < endIndex; ++i) {
					...
					let isLastFragmentOnLine =
						nextFragment === undefined || thisFragment.line !== nextFragment.line;
 ...
 */
function* eachObjectOnLine(
	line: number,
	guts: LayoutGuts<WithFragmentRanges>
): IterableIterator<FragmentContent | HGadget> {
...
function addVGadget(
	...
	let offset = 0;
	let itree: IntervalTree<VGadget> = side === 'Above' ? line.aboveLine : line.belowLine;
 ...
 * `DrawCommands`.
 */
function resolveWidths<A>(
	layoutGuts: LayoutGuts<WithFragmentRanges<A>>
): LayoutGutsWithLeading<WithFragmentRanges<WithDrawCommands<A>>> {
	/**
	 * An array, one element per line. Each line has two interval trees;
	 * a tree for the VGadgets above the line, and a tree for the
	 * VGadgets below.
	 */
	const leading: Leading = layoutGuts.lineToFragmentRange.map((_) => ({
	...
	const go = (
		root: LayoutTree<WithFragmentRanges<A>>
	): LayoutTree<WithFragmentRanges<WithDrawCommands<A>>> => {
		switch (root.type) {
			case 'Newline':
				return root;
			case 'Atom':
			...
			case 'Node': {
				...
				return {
					...root,
					children,
					drawCommands: [topRight, topLeft, bottomLeft, bottomRight, closeCommand()]
				};
			}
		}
	};
...
function resolveHeights(
	layoutGuts: LayoutGutsWithLeading<WithMeasurements<WithFragmentRanges<WithDrawCommands>>>,
	idealLeading: number
): LayoutTree<WithMeasurements<WithFragmentRanges<WithOutlines>>> {
	let y = 0;
	/**
	...
	y = 0;
	const go = (
		root: LayoutTree<WithMeasurements<WithFragmentRanges<WithDrawCommands>>>
	): LayoutTree<WithMeasurements<WithFragmentRanges<WithOutlines>>> => {
		switch (root.type) {
			case 'Newline':
				return root;
	...
	boundingBox(): Rect | null {
		switch (this.layoutTree.type) {
			case 'Newline':
				return null;
			case 'Atom':
				return clone(this.layoutTree.rect);
			case 'Spacer':
				return null;
			case 'Node':
				return new PolygonRendering(this.layoutTree.outline).boundingBox();
		}
	}
