...
export function eachRectiSegmentTriple<A>(
	...
	f: (
		...
		curt: RectiSegment,
		next: RectiSegment
	) => EachRectiSegmentIterationDecision<A>
): A | null {
	if (path.length <= 2) {
		return null;
	}
...
function mkRectiSegment(begin: Point, end: Point): RectiSegment {
	...
	if (dx > 0.001 && dy > 0.001) {
		throw new Error(
			`Non axis-aligned points in mkRectiSegment (${begin.x}, ${begin.y}) -- (${end.x}, ${end.y})`
		);
	}
...
export function pointInPath(
	p: Point,
	path: Path,
	includeEdges: 'IncludeEdges' | 'ExcludeEdges'
): boolean {
...
export function rectPathIntersectionArea(rect: Rect, path: Path): number {
	...
	for (const event of events) {
		...
		const intervalArea =
			Math.abs(event.interval[0] - event.interval[1]) * Math.abs(rect.bottom - event.yCoordinate);
 ...
 * otherwise.
 */
export function tryToRemoveAntiknobs(
	path: Path,
	keepInside?: Polygon,
	keepOutside?: Polygon
): boolean {
	...
	eachRectiSegmentTriple(path, (a, b, c) => {
		...
		const newArea = dNearBack < dFarBack ? rectOfRectiSegments(a, b) : rectOfRectiSegments(b, c);
 ...
 * otherwise.
 */
export function tryToRemoveClockwiseCorners(
	path: Path,
	keepInside?: Polygon,
	keepOutside?: Polygon
): boolean {
	...
	for (let i = 0; i < path.length; ++i) {
		...
		if (shouldDrawAB && shouldDrawBC) {
			...
			if (actualRadius === 0) {
				...
				out += `L ${arcBegin.x} ${arcBegin.y} A ${actualRadius} ${actualRadius} 0 0 ${
					isTurnCW(a, b, c) ? '1' : '0'
				} ${arcEnd.x} ${arcEnd.y} `;
			}
