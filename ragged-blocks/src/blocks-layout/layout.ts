...
function outermostRect(layoutTree: rlt.LayoutTree<rlt.WithPositions>): Rect | null {
	switch (layoutTree.type) {
		case 'JoinH':
		case 'JoinV':
			return outermostRect(layoutTree.lhs) || outermostRect(layoutTree.rhs);
		case 'Atom':
			return layoutTree.rect;
		case 'Spacer':
			return null;
		case 'Wrap':
			return layoutTree.rect;
...
export default class BlocksLayout implements alt.Layout {
	...
	async layout(layoutTree: alt.LayoutTree<alt.WithMeasurements>): Promise<BlocksLayoutResult> {
		 ...
		 */
		const goRel = (
			root: rlt.LayoutTree<rlt.WithMeasurements>
		): [rlt.LayoutTree<WithRelativeOffsets>, Rect] => {
			switch (root.type) {
				case 'JoinH': {
		 ...
		 */
		const goFinalize = (
			root: rlt.LayoutTree<WithRelativeOffsets>,
			ofs: Vector
		): rlt.LayoutTree<rlt.WithPositions> => {
			switch (root.type) {
				case 'JoinH':
				case 'JoinV': {
