...
test('Can reassoc a list of Atoms', () => {
	const t: alt.LayoutTree = node([aatom(), aatom(), aatom()]);

	expect(reassocLayoutTree(t, empty)).toStrictEqual(wrap(joinh(joinh(ratom(), ratom()), ratom())));
});

test('Can reassoc a list of Atoms separated by a Newline', () => {
	const t: alt.LayoutTree = node([aatom(), aatom(), newline(), aatom()]);

	expect(reassocLayoutTree(t, empty)).toStrictEqual(wrap(joinv(joinh(ratom(), ratom()), ratom())));
});
...
test('Can reassoc a list of Atoms separated by a Newlines', () => {
	...
	expect(reassocLayoutTree(t, empty)).toStrictEqual(
		wrap(joinv(joinv(joinh(ratom(), ratom()), ratom()), joinh(joinh(ratom(), ratom()), ratom())))
	);
});

test('Can reassoc two Newlines in a row', () => {
	const t: alt.LayoutTree = node([aatom(), newline(), newline(), aatom()]);

	expect(reassocLayoutTree(t, empty)).toStrictEqual(wrap(joinv(joinv(ratom(), ratom()), ratom())));
});

test('Can reassoc a tree beginning with a Newline', () => {
	const t: alt.LayoutTree = node([newline(), newline(), aatom()]);

	expect(reassocLayoutTree(t, empty)).toStrictEqual(wrap(joinv(joinv(ratom(), ratom()), ratom())));
});

test('Can reassoc trailing Newlines', () => {
	const t: alt.LayoutTree = node([newline(), newline(), newline()]);

	expect(reassocLayoutTree(t, empty)).toStrictEqual(
		wrap(joinv(joinv(joinv(ratom(), ratom()), ratom()), ratom()))
	);
});
