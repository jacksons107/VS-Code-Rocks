...
import {
	intervalsIntersect,
	subIntervalInterval,
	subIntervalIntervals,
	fromRectangles
} from '../../src/polygon/from-rectangles';
...
test('Can subtract two intervals', () => {
	expect(subIntervalInterval([3.33, 6.1], [3, 3.66])).toStrictEqual([[3.66, 6.1]]);

	expect(subIntervalInterval([1.5, 1.8], [1.8, 2.0])).toStrictEqual([[1.5, 1.8]]);

	expect(subIntervalInterval([1.0, 2.0], [10.0, 20.0])).toStrictEqual([[1.0, 2.0]]);

	expect(subIntervalInterval([1.0, 2.0], [-10.0, 20.0])).toStrictEqual([]);

	expect(subIntervalInterval([-3.1, 3.4], [-1.0, 1.2])).toStrictEqual([
		[-3.1, -1.0],
		[1.2, 3.4]
	]);
});
...
test('Can find the polygon for a single rectangle', () => {
	const pgon = fromRectangles([r(0, 0, 10, 10)]);
...
test('Can find the polygon for two disjoint rectangles', () => {
	const pgon = fromRectangles([r(0, 0, 10, 10), r(20, 20, 10, 10)]);
...
test('Can find the polygon for two stacked rectangles (just touching)', () => {
	const pgon = fromRectangles([r(0, 0, 10, 10), r(0, 10, 10, 10)]);
	expect(pgon.length).toBe(1);
...
test('Can find the polygon for two stacked rectangles (overlapping)', () => {
	const pgon = fromRectangles([r(0, 0, 10, 10), r(0, 5, 10, 10)]);
	expect(pgon.length).toBe(1);
...
test('Can find the polygon for two offset rectangles', () => {
	const pgon = fromRectangles([r(0, 0, 10, 10), r(5, 5, 10, 10)]);
	expect(pgon.length).toBe(1);
...
test('Can find the polygon for a triangle pattern', () => {
	...
	// |  +--+  |
	// +--+  +--+
	const pgon = fromRectangles([r(5, 0, 5, 15), r(0, 10, 5, 10), r(10, 10, 5, 10)]);
	expect(pgon.length).toBe(1);
...
test('Can find the polygon for an inverted triangle pattern', () => {
	...
	//    |  |
	//    +--+
	const pgon = fromRectangles([r(0, 0, 5, 10), r(10, 0, 5, 10), r(5, 5, 5, 15)]);
	expect(pgon.length).toBe(1);
