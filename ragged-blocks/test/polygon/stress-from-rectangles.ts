...
class Counterexample extends Render {
	...
	render(svg: Svg) {
		...
		svg
			.circle(5)
			.move(this.p.x - 5 / 2, this.p.y - 5 / 2)
			.fill('magenta');
	}
...
function reduceExample(rs: Rect[], samplePoint: Point): [Rect[], Polygon] {
	for (const subset of allSubsets(rs)) {
		...
		if (ref !== test) {
			console.log(
				`Found a counterexample (rs ${ref ? 'inside' : 'outside'} while pgon ${
					test ? 'inside' : 'outside'
				})`
			);
			return [subset, pgon];
			...
			for (let j = 0; j < 1000; ++j) {
				...
				if (ref !== test) {
					...
					if (rsReduced.length !== rs.length) {
						console.log(
							`Reduced example from ${rs.length} rectangles to ${rsReduced.length} rectangles.`
						);
					}
...
function runExample(path: string) {
	...
	if (ref !== test) {
		console.log(
			`Example failed (rs ${ref ? 'inside' : 'outside'} while pgon ${test ? 'inside' : 'outside'})`
		);
	} else {
