...
function escapeAnd(s: string): Parsimmon.Parser<string> {
	return Parsimmon.seq(Parsimmon.string('\\'), Parsimmon.string(s)).map(([_, s]) => s);
}

const pText: Parsimmon.Parser<string> = Parsimmon.alt(
	escapeAnd('#'),
	escapeAnd('@'),
	escapeAnd('['),
	escapeAnd(']'),
...
const pSpaceAtom: Parsimmon.Parser<LayoutTree> = Parsimmon.string(' ')
	.atLeast(1)
	.map((s) => {
...
const pAtom: Parsimmon.Parser<LayoutTree> = Parsimmon.seq(
...
const pNewline: Parsimmon.Parser<LayoutTree[]> = Parsimmon.seq(
	Parsimmon.lf,
	Parsimmon.regexp(/ */)
).map(([_, ws]) => {
	if (ws.length === 0) {
...
const pName: Parsimmon.Parser<string> = Parsimmon.regexp(/[a-z][a-zA-Z0-9-\_]*/);

const pStyleReference: Parsimmon.Parser<string> = Parsimmon.seq(Parsimmon.string('@'), pName).map(
	([_, nm]) => nm
);
...
const pNumber: Parsimmon.Parser<number> = Parsimmon.seq(
...
const pRGBAColor: Parsimmon.Parser<string> = Parsimmon.seq(
	pKeyword('rgba('),
	pNumber.times(4),
	pKeyword(')')
).map((strings) => strings.join(''));

const pColor: Parsimmon.Parser<string> = Parsimmon.alt(
	Parsimmon.regexp(/#[\da-f]+/i),
	pRGBAColor,
	pName
).skip(Parsimmon.optWhitespace);

function pKeyValuePair<A, B>(
	k: Parsimmon.Parser<A>,
	v: Parsimmon.Parser<B>
): Parsimmon.Parser<[A, B]> {
	return Parsimmon.seq(k.skip(pKeyword(':')), v.skip(pKeyword(';')));
}
...
const pSide: Parsimmon.Parser<Partial<BorderStyle>> = Parsimmon.alt(
	pKeyword('top').map((_) => ({ borderTop: true })),
	pKeyword('bottom').map((_) => ({ borderBottom: true })),
	pKeyword('left').map((_) => ({ borderLeft: true })),
	pKeyword('right').map((_) => ({ borderRight: true }))
);

const pSides: Parsimmon.Parser<Partial<BorderStyle>> = pSide.many().map((sides) => {
...
const pBorderSpec: Parsimmon.Parser<Partial<BorderStyle>> = pKeyValuePair(
	pKeyword('border'),
	pBorderSpecValue
).map(([_, v]) => v);

const pStyleAttr: Parsimmon.Parser<Style> = Parsimmon.alt(
...
const pStyles: Parsimmon.Parser<Style> = Parsimmon.seq(
	pKeyword('{'),
	pStyleAttr.many(),
	pKeyword('}')
).map(([_l, attrs, _r]) => {
...
const pStyleDefn: Parsimmon.Parser<[string, Style]> = Parsimmon.seq(
	pKeyword('@'),
...
const pExample: Parsimmon.Parser<Example> = Parsimmon.seq(
	pLayoutTree.many(),
	pStyleDefn.many()
