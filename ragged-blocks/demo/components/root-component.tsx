...
export default function Root() {
	...
	const [activeLayouts, _setActiveLayouts] = react.useState<rb.AlgorithmName[]>(() => {
		...
		if (Array.isArray(res) && res.every((elem) => typeof elem === 'string')) {
			return res.map(rb.asAlgorithmName).filter((elem) => elem !== undefined);
		}
	...
	const hasLayout = react.useCallback(
		(algoName: rb.AlgorithmName) => activeLayouts.some((algo) => algo === algoName),
		[activeLayouts]
	);

	function removeLayout(algoName: rb.AlgorithmName) {
		setActiveLayouts((activeLayouts) => activeLayouts.filter((algo) => algo !== algoName));
	}
		...
		<div>
			<div className={styles.row}>
				<span className={styles.label}>
					View on <a href={'https://github.com/sbcohen2000/ragged-blocks'}>GitHub</a>
				</span>
			...
			<Dropdown isOpen={aboutOpen} onChange={setAboutOpen} label={'About'}>
				<p className={styles.aboutText}>
					This website is an interactive sandbox that accompanies our paper,{' '}
					<a href={'https://arxiv.org/pdf/2507.06460'}>
						<em>Ragged Blocks: Rendering Structured Text with Style</em>
					</a>
					. It implements all of the algorithms that we benchmarked in the paper, and offers a way
					to visualize how each algorithm renders a layout tree of the reader's choice.
					<br />
					<br />
					See the below <em>Syntax Guide</em> for instructions on how to construct your own
					examples. Type into the <em>Playground</em> to construct examples, and compare the outputs
					of different layout algorithms.
				</p>
			</Dropdown>
			<div className={styles.sectionLine}></div>
			...
			<div className={styles.row}>
				...
				<Tooltip>
					<div className={styles.tooltipContent}>
						If enabled, layout and SVG generation will be scheduled on a Web Worker, rather than on
						the main thrd. This permits multiple algorithms to layout in parallel.
					</div>
					<LabeledCheckbox
			...
			<div className={styles.compareButtonsCluster}>
				<span className={styles.label}>Layout Algorithms</span>
				<div className={styles.compareButtons}>
					<Tooltip>
						<div className={styles.tooltipContent}>
							This algorithm is as close as possible to the formal description from the paper. It
							does not use <em>Stateful Regions</em>, however, and so does not render polygonal
							outlines around each Rock.
						</div>
						<Button label={'L1P'} onClick={() => toggleLayout('L1P')} enabled={hasLayout('L1P')} />
					</Tooltip>
					<Tooltip>
						<div className={styles.tooltipContent}>
							Algorithm L1P, extended with stateful regions (<em>S</em>) and simplification (
							<em>+</em>).
						</div>
						<Button
							label={'L1S+'}
					...
					<Tooltip>
						<div className={styles.tooltipContent}>
							Blocks renders each node in the Layout Tree inside a rectangle. This can lead to
							unnatural text layouts which don't resemble their source. However, this algorithm is
							very simple and pretty fast.
						</div>
						<Button
							label={'Blocks'}
					...
					<Tooltip>
						<div className={styles.tooltipContent}>
							Our previous algorithm, described in <em>Code Style Sheets: CSS For Code</em>. It is
							more performant than L1S+, but cannot always produce layouts of the same compactness.
						</div>
						<Button
							label={'S-Blocks'}
