		...
		<div className={styles.layoutContainer} key={props.algoName}>
			<div className={styles.upperHalf}>
				<div className={styles.buttonColumn}>
					<Tooltip>
						<div className={styles.tooltipContent}>Download this layout as an SVG.</div>
						<Button label={''} icon={faDownload} onClick={downloadLayout} />
					</Tooltip>
				</div>
				<div>
			...
			<Dropdown isOpen={settingsOpen} icon={faGear} onChange={setSettingsOpen}>
				<LabeledCheckbox
...
function LayoutResultView({ layoutResult }: { layoutResult: LayoutResult }) {
	...
	if (layoutResult.status === 'done') {
		...
		return <div dangerouslySetInnerHTML={{ __html: layoutResult.svgSrc }}></div>;
	} else {
		return (
			<div className={styles.stack}>
				{lastGoodResult.current !== null ? (
					<div
						className={styles.lastGoodResult}
